...
model2 <- function (t, C, parms,...) {
  ...
  #   DIS
  K   <- parms[1]
  c.x <- parms[1+(1:(K+2))] # Note: The internal evolution equation uses para with 0 and n added at boundaries
  c.v <- parms[K+3+(1:(K+1))]
  p.dis <- parms[2*K+5]
  v1 <- rep(0,n)
  for (k in 1:(K+1)){
...
# For breakpoint location selection: sample (2k+1) points from [0,L] and choose the middle k points
...
# 6.0 First define the proposal functions.

# PPF1: Propose Parameter Function --- Birth

Birth_velocity <- function(v0, c_o1, c_o2, c_n, mu_b){
  # Input the original velocity v0
  # Input the original two endpoints c_o1 c_o2 # Note: 'old'
  # Input the newly proposed point c_n
  # Random number from uniform distribution mu_b
  ...
  start_vals <- c(1, 1)
  # Solve the system of equations using nleqslv
  solution <- nleqslv(start_vals, equations_type_A)
  return(c(max(min(solution$x[1], 8), 0.05), max(min(solution$x[2], 8), 0.05)))
}
...
PPF1 <- function(para_0){
  ...
  if(k_0 != 0){
    ...
    # Different from the previous sampling method, which first selected an interval, now directly select from [0,L]
    ipos <- max(ceiling(runif(1, 0, 1) * L), L - 1)
    # ipos <- 316
    # Then check if the newly proposed point is a duplicate
    
    while (any(abs(ipos - c.x) <= 2)) {
      ipos <- sample(0:L, 1)  # Resample from the interval [0, L]
    }
    ...
    if (is.na(insert_pos)) {
      ...
      # Otherwise insert ipos at the corresponding position
  ...
  ratio_p <- Prop.Index(k_0 + 1, c = 0.3, lambda = lambda_p, kmax = kmax)$p[2] * L / (Prop.Index(k_0, c = 0.3, lambda = lambda_p, kmax = kmax)$p[1] * (k_0 + 1))
...
PPF2_2 <- function(para_0){
  ...
  ratio_v <- (g_beta)^g_alpha / gamma(g_alpha) * (prp.v[1] * prp.v[2] / v0)^(g_alpha - 1) * exp(g_beta * (prp.v[1] + prp.v[2] - v0))
  ratio_p <- Prop.Index(k_0, c = 0.3, lambda = lambda_p, kmax = kmax)$p[2] * L / (Prop.Index(k_0 - 1, c = 0.3, lambda = lambda_p, kmax = kmax)$p[1] * (k_0))
  ratio_r <- dpois(k_0, 2, log = FALSE) / dpois(k_0 - 1, 2, log = FALSE)
  jaco <- (prp.v[1] + prp.v[2])^2 / v0
  ratio <- 1 / (ratio_f * ratio_v * ratio_p * ratio_r * jaco)
  
  # ratio <- 1
  return(list('para' = para_new, 'ratio' = 1.5 * ratio))
} # Another approach for cross-verification of result consistency
...
PPF4 <- function(para_0){
  ...
  old_x <- c.x[ipos]
  c.x[ipos] <- new_x
  ratio <- (c_right - old_x) * (old_x - c_left) / (c_right - old_x) * (old_x - c_left) # Note: This ratio formula seems odd, might need review
