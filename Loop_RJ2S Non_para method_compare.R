...
# Compare 3 estimations for parameter -------------------------------------
...
for(i.all in 1:N_Loop){
  save(Res_ALL,  file = paste0(R_path,"--------Loop i.all =",i.all," start.RData"))
  set.seed(i.all+1006+1024)
  lambda_p <- 2 #泊松先验的均值
  kmax <- 3
  N <- 60
  times <- 0:600
  ...
  Res1_NP <- DA_Process_NonP(Enstat,EnPara,1,1)
  ...
  Res1_PE <- DA_Process_EstP(Enstat_PE,EnPara_PE,1)
...
Res_pm <- list(Res_ALL,ESS_1RJ,ESS_2NP,ESS_3PE,ACP_1RJ,ACP_2NP,ACP_3PE)
save(Res_pm,  file = paste0("NewRes_3para_esti_method (N=",N,").RData"))
colMeans(Res_ALL)
#colMeans(Res_ALL[c(-6,-8),])
...
data <- data.frame(
  ...
  Preset_Distribution =      rowMeans(Mtx.ESS_1),
  Nonparametric_Estimation = rowMeans(Mtx.ESS_2),
  Parametric_Estimation =    rowMeans(Mtx.ESS_3)
)
...
ggplot(data_long, aes(x = Time, y = ESS, color = Method)) +
  ...
  scale_color_manual(
    ...
    labels = c("Preset Distribution", "Nonparametric Estimation", "Parametric Estimation")
  ) +
  labs(
...
data <- data.frame(
  ...
  Preset_Distribution =      rowMeans(Mtx.ACP_1),
